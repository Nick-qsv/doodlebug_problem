...
void Ant::move(string (&grid)[ROWS][COLS]) {
    int x = this->getX();
    int y = this->getY();
...
void Ant::breed(string (&grid)[ROWS][COLS], vector<Doodlebug>& new_db, vector<Ant>& new_a) {
    int x = this->getX();
    int y = this->getY();
    if (valid_move(x, y + 1) && grid[x][y + 1].empty()) {
        Ant ant(x, y + 1);
...
void Doodlebug::breed(string (&grid)[ROWS][COLS], vector<Doodlebug>& new_db, vector<Ant>& ants) {
    int x = this->getX();
    int y = this->getY();
    if (valid_move(x, y + 1) && grid[x][y + 1].empty()) {
        Doodlebug db(x, y + 1);
